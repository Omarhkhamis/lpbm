...
const getRoleLabel = (role) =>
  normalizeAdminRole(role) === ADMIN_ROLE.DATA_VIEWER
    ? "Data only"
    : "Full admin";

const getDataAccessLabel = (value) => {
  const access = normalizeDataLocaleAccess(value);
  if (access === DATA_LOCALE_ACCESS.RU) return "Russian";
  if (access === DATA_LOCALE_ACCESS.EN) return "English";
  return "English + Russian";
};

export default async function AdminUsersPage({ searchParams }) {
  const site = normalizeSite(searchParams?.site) || "hollywood-smile";
  await requireFullAdmin(site);
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              className="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-copper-400 focus:ring-1 focus:ring-copper-400/40"
            >
              <option value={ADMIN_ROLE.ADMIN}>Full admin</option>
              <option value={ADMIN_ROLE.DATA_VIEWER}>Data only</option>
            </select>
          </div>
          <div>
            <label className="text-xs uppercase tracking-[0.2em] text-slate-500">
            ...
            <select
              name="dataLocaleAccess"
              defaultValue={DATA_LOCALE_ACCESS.ALL}
              className="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-copper-400 focus:ring-1 focus:ring-copper-400/40"
            >
              <option value={DATA_LOCALE_ACCESS.ALL}>English + Russian</option>
              <option value={DATA_LOCALE_ACCESS.EN}>English only</option>
              <option value={DATA_LOCALE_ACCESS.RU}>Russian only</option>
            </select>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {admins.map((admin) => (
                <tr key={admin.id} className="text-slate-700 align-top">
                  ...
                  <td className="px-6 py-4">{getRoleLabel(admin.role)}</td>
                  <td className="px-6 py-4">
                    {getDataAccessLabel(admin.dataLocaleAccess)}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    ...
                    >
                      ...
                      <select
                        name="role"
                        defaultValue={normalizeAdminRole(admin.role)}
                        className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-copper-400 focus:ring-1 focus:ring-copper-400/40"
                      >
                        <option value={ADMIN_ROLE.ADMIN}>Full admin</option>
                        <option value={ADMIN_ROLE.DATA_VIEWER}>Data only</option>
                      ...
                      <select
                        name="dataLocaleAccess"
                        defaultValue={normalizeDataLocaleAccess(
                          admin.dataLocaleAccess
                        )}
                        className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-copper-400 focus:ring-1 focus:ring-copper-400/40"
                      >
                        <option value={DATA_LOCALE_ACCESS.ALL}>
                          English + Russian
                        </option>
                        <option value={DATA_LOCALE_ACCESS.EN}>English only</option>
                        <option value={DATA_LOCALE_ACCESS.RU}>Russian only</option>
