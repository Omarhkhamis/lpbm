...
const resolveThankYouUrl = (payload = {}, serverUrl) => {
  if (typeof serverUrl === "string" && serverUrl.trim()) {
    return serverUrl.trim();
  }
  const page = String(payload?.page || "");
  const locale =
    String(payload?.locale || "").toLowerCase() === "ru" ||
    page.includes("/ru")
      ? "ru"
      : "en";
  const site = payload?.site || detectSiteFromPath(page) || "hollywood-smile";
  return `/thankyou?site=${encodeURIComponent(site)}&locale=${encodeURIComponent(locale)}`;
};
...
export const submitFormPayload = async (payload, options = {}) => {
  ...
  try {
    ...
    if (typeof window !== "undefined" && !skipRedirect && !isSpin) {
      ...
      const thankYouUrl = resolveThankYouUrl(basePayload, data?.thankYouUrl);
