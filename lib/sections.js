...
import {
  getSectionDefaults,
  normalizeImplantMatrixDataForLocale
...
const ensureLocaleSections = async (site, locale) => {
  ...
  for (const def of definitions) {
    ...
    if (def.key === "implantMatrix" && lang === "ru") {
      const normalizedData = normalizeImplantMatrixDataForLocale(found.data, lang);
      if (normalizedData !== found.data) {
        updateData.data = normalizedData;
      }
    }
